...
struct Value {
        ...
        if (type == ValueType::NUMBER) {
            ...
            if (num_val >= -9007199254740992.0 && num_val <= 9007199254740992.0) {
                if (num_val == std::floor(num_val)) {
                    ...
                    oss << std::fixed; oss.precision(0); oss << num_val;
                    return oss.str();
                }
            }
            std::ostringstream oss;
            oss << std::fixed; oss.precision(16); oss << num_val;
    ...
    double to_number() const {
        ...
        if (type == ValueType::STRING) {
            try {
                size_t idx; double res = std::stod(str_val, &idx); return res;
            } catch (...) { return 0.0; }
        }
        return 0.0;
    }
...
class Parser {
    ...
    Value primary() {
        ...
        if (t.type == TokenType::IDENTIFIER) {
                ...
                if (peek().type == TokenType::DOT) {
                    ...
                    if (member.type == TokenType::IDENTIFIER && member.text == "number") {
                        ...
                        if (peek().type == TokenType::LPAREN) {
                            advance(); Value prompt("");
                            if (peek().type != TokenType::RPAREN) prompt = expression();
                            if (peek().type == TokenType::RPAREN) advance(); 
                            
                            std::cout << prompt.to_string() << std::flush;
                            std::string user_in;
                            if (!std::getline(std::cin, user_in)) return Value(0.0);
                            if (!user_in.empty() && user_in.back() == '\r') user_in.pop_back(); 
                            try {
                                size_t idx = 0;
                    ...
                    if (std::getline(std::cin, user_in)) {
                        ...
                        if (first != std::string::npos && last != std::string::npos) {
                            user_in = user_in.substr(first, (last - first + 1));
                        } else user_in = "";
                        try {
                            size_t idx = 0; double num = std::stod(user_in, &idx);
                            if (idx == user_in.size()) return Value(num);
                        } catch (...) {}
            ...
            if (state.libraries.count(t.text)) {
                ...
                if (peek().type == TokenType::LPAREN) {
                    ...
                    try { size_t idx; double d = std::stod(result, &idx); if (idx == result.size()) return Value(d); } 
                    catch (...) {}
                    return Value(result);
                } else {
                    return Value(0.0);
        ...
        if (t.type == TokenType::NUMBER) { advance(); return Value(std::stod(t.text)); }
        if (t.type == TokenType::STRING) { advance(); return Value(t.text); }
        if (t.type == TokenType::TRUE_LIT) { advance(); return Value(true); }
        if (t.type == TokenType::FALSE_LIT) { advance(); return Value(false); }
        if (t.type == TokenType::LPAREN) {
            advance(); Value val = expression();
            if (peek().type == TokenType::RPAREN) advance(); return val;
        }
        if (t.type == TokenType::NOT) { advance(); return Value(!primary().to_bool()); }
        return Value(0.0);
    }
    ...
    Value factor() {
        ...
        while (peek().type == TokenType::MULTIPLY || peek().type == TokenType::DIVIDE || peek().type == TokenType::MODULO) {
            Token op = advance(); Value right = primary();
            if (op.type == TokenType::MULTIPLY) val = Value(val.to_number() * right.to_number());
    ...
    Value comparison() {
        ...
        while (peek().type == TokenType::EQUAL || peek().type == TokenType::LESS ||
            ...
            else if (op.type == TokenType::LESS) val = Value(val.to_number() < right.to_number());
            else if (op.type == TokenType::GREATER) val = Value(val.to_number() > right.to_number());
            else if (op.type == TokenType::LESSEQUAL) val = Value(val.to_number() <= right.to_number());
            else if (op.type == TokenType::GREATEREQUAL) val = Value(val.to_number() >= right.to_number());
        }
        return val;
    }
    ...
    Value logical_and() {
        ...
        while (peek().type == TokenType::AND) { advance(); Value right = comparison(); val = Value(val.to_bool() && right.to_bool()); }
        return val;
    }
...
// ==========================================
// Terminal & Parsing Utilities
// ==========================================
bool check_key_pressed(const std::string& key_name) {
    struct termios orig_t; if (tcgetattr(STDIN_FILENO, &orig_t) == -1) return false;
    struct termios raw = orig_t; raw.c_lflag &= ~(ECHO | ICANON); raw.c_cc[VMIN] = 0; raw.c_cc[VTIME] = 0;
    ...
    if (ret > 0 && FD_ISSET(STDIN_FILENO, &fds)) {
        ...
        if (read(STDIN_FILENO, &ch, 1) > 0) {
            if (key_name == "space" && ch == ' ') pressed = true;
            else if (key_name == "enter" && ch == '\n') pressed = true;
            else if (key_name.size() == 1 && ch == key_name[0]) pressed = true;
            else if (ch == 27) { 
                char seq[2];
...
Value evaluate(std::string expr, Scope& local_vars, Scope& global_vars, ExecutionState& state) {
    ...
    while ((event_pos = expr.find("event:")) != std::string::npos) {
        ...
        while (end_pos < expr.size() && (std::isalnum(expr[end_pos]) || expr[end_pos] == '-' || expr[end_pos] == '.' || expr[end_pos] == ':')) end_pos++;
...
std::string strip_comments_and_normalize(std::string line) {
    ...
    for (size_t i = 0; i < line.size(); i++) {
        if ((line[i] == '"' || line[i] == '\'') && (i == 0 || line[i-1] != '\\')) {
            if (!in_string) { in_string = true; string_char = line[i]; } 
            else if (line[i] == string_char) in_string = false;
...
std::vector<std::shared_ptr<ASTNode>> parse_blocks(const std::vector<std::string>& lines) {
    ...
    for (const auto& line : lines) {
        ...
        while (i < line.size() && (line[i] == ' ' || line[i] == '\t')) {
            indent_val += (line[i] == '\t' ? 4 : 1); i++;
        }

        std::string stripped = line.substr(i); 

        while (stack.size() > 1 && stack.back().first >= static_cast<int>(indent_val)) {
            stack.pop_back();
        }
...
std::vector<std::string> preprocess_code(const std::string& code) {
    ...
    while (std::getline(ss, current_line)) {
        ...
        bool in_str = false; char str_c = '\0';
        for (char c : line) {
            if (c == '"' || c == '\'') {
...
const std::unordered_set<std::string> RESERVED_KEYWORDS = {
    "globalvar", "localvar", "globvar", "function", "dofunc", "if", "else", "elseif", "until", 
    "while", "repeat", "addlib", "try", "catch", "exitloop", "wait", "warn", "critical", "exit", "env"
};
...
void pre_run_checks(const std::vector<std::shared_ptr<ASTNode>>& nodes, const ExecutionState& state) {
    for (const auto& node_ptr : nodes) {
        std::string line = node_ptr->line;
        ...
        if (!node_ptr->children.empty()) pre_run_checks(node_ptr->children, state);
    }
}
    ...
    if (os_release.is_open()) {
        ...
        while (std::getline(os_release, line)) {
            if (line.rfind("ID=", 0) == 0 || line.rfind("ID_LIKE=", 0) == 0) {
                for (char& c : line) c = std::tolower(c); combined += line + " ";
            }
        }
...
std::string get_env_directory() {
    ...
    return home ? std::string(home) + "/.vex_environments" : ".vex_environments";
}

std::string get_lib_directory() {
    const char* home = std::getenv("HOME");
    return home ? std::string(home) + "/.vex_libraries" : ".vex_libraries";
}
...
void ensure_default_main_env() {
    ...
    if (stat(main_path.c_str(), &st) != 0) {
        ...
        if (f.is_open()) f << "{\"name\": \"main\",\"max_mem_usage\": null,\"max_cpu_usage\": null,\"blocked_statements\": \"\"}\n";
    }
}
    ...
    if (stat(filepath.c_str(), &st) == 0) {
        ...
        if (f.is_open()) {
            ...
            if (std::regex_search(raw_json, m, blocked_regex)) {
                std::string blocked_str = m[1].str(); std::stringstream ss(blocked_str); std::string item;
                state.blocked_statements.clear();
                while (std::getline(ss, item, ',')) {
                    size_t first = item.find_first_not_of(" \t"); size_t last = item.find_last_not_of(" \t");
                    if (first != std::string::npos && last != std::string::npos) state.blocked_statements.push_back(item.substr(first, (last - first + 1)));
                }
            }
            std::regex mem_regex("\"max_mem_usage\"\\s*:\\s*(\\d+)");
            if (std::regex_search(raw_json, m, mem_regex)) {
                long long limit_mb = std::stoll(m[1].str());
                rlimit rl; rl.rlim_cur = limit_mb * 1024 * 1024; rl.rlim_max = rl.rlim_cur; setrlimit(RLIMIT_AS, &rl);
            }
            std::regex cpu_regex("\"max_cpu_usage\"\\s*:\\s*(\\d+)");
            if (std::regex_search(raw_json, m, cpu_regex)) {
                int cpu_percent = std::stoi(m[1].str());
                if (cpu_percent < 100) nice((100 - cpu_percent) * 19 / 100);
            }
        }
    } else {
        if (explicit_flag) { std::cerr << "Environment Error: '" << env_name << "' not found.\n"; std::exit(1); }
        state.blocked_statements.clear();
    }
}
...
void ensure_library(const std::string& libname) {
    ...
    if (stat(exec_path.c_str(), &st) != 0) {
        ...
        if (system(("mkdir -p " + lib_dir).c_str()) != 0) {}

        std::string base_url = "https://raw.githubusercontent.com/AstroMeYT/Vex/main/VexLibC/" + libname + "/";
        if (system(("curl -s -o " + json_path + " " + base_url + "library-info.json").c_str()) != 0) {}
        
        std::ifstream jf(json_path);
        if (!jf.is_open()) { std::cerr << "Error: Library not found.\n"; std::exit(1); }
        ...
        if (system(("curl -s -o " + exec_path + " " + base_url + libname).c_str()) != 0) {}
        if (stat(exec_path.c_str(), &st) != 0) { std::cerr << "Download failed.\n"; std::exit(1); }
        
        chmod(exec_path.c_str(), 0755); 
        std::cout << "Library '" << libname << "' installed.\n" << std::flush;
    }
}
...
// ==========================================
std::string run_ast(const std::vector<std::shared_ptr<ASTNode>>& nodes, Scope& local_vars, Scope& global_vars, 
                    std::unordered_map<std::string, std::vector<std::shared_ptr<ASTNode>>>& functions, ExecutionState& state) {
    ...
    for (const auto& node_ptr : nodes) {
        ...
        if (line.rfind("addlib ", 0) == 0) {
            std::string rem = line.substr(7); rem.erase(0, rem.find_first_not_of(" \t"));
            std::string libname, alias; size_t as_pos = rem.find(" as ");
            if (as_pos != std::string::npos) {
                libname = rem.substr(0, as_pos); alias = rem.substr(as_pos + 4);
                libname.erase(libname.find_last_not_of(" \t") + 1); alias.erase(0, alias.find_first_not_of(" \t"));
            } else { libname = rem; alias = libname; }
            ...
            for (char c : args_str) {
                if (c == '"' || c == '\'') {
                    ...
                    else if (c == ',' && paren_depth == 0) { parts.push_back(current); current = ""; continue; }
                }
                current += c;
            }
            ...
            if (std::regex_search(line, match, var_reg)) {
                std::string vtype = match[1].str(); std::string name = match[2].str();
                std::string op = match[3].str(); std::string expr = match[4].str();
            ...
            try { std::string res = run_ast(children, local_vars, global_vars, functions, state); if (!res.empty()) return res; } 
            catch (...) { last_try_failed = true; }
        }
        else if (line == "catch") {
            if (last_try_failed) {
                std::string res = run_ast(children, local_vars, global_vars, functions, state);
...
void run_environment_tui() {
    ...
    if (name.empty()) { std::cerr << "Error: Environment name cannot be empty.\n"; std::exit(1); }
    std::cout << "MAX-MEM-USAGE (MB) > "; std::getline(std::cin, mem);
    std::cout << "MAX-CPU-USAGE (%) > "; std::getline(std::cin, cpu);
    std::cout << "BLOCKED-STATEMENTS (comma-separated) > "; std::getline(std::cin, blocked);
